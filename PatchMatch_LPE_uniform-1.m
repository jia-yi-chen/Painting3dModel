...
iteration=0;
T=25;%errorness detection
K=2;
debug.target_covered=zeros(tsz(1)+2*w,tsz(2)+2*w);
debug.source_used=zeros(ssz(1),ssz(2));
% for iteration = 1:max_iterations
while 1
    %需要保证一直能找到match，(debug.source能一直平均增加）
    if(sum(sum(debug.target_covered))/((tsz(1)+2*w)*(tsz(2)+2*w))<0.95)
        iteration=iteration+1;
        is_odd = mod(iteration,2)==1;
        disp([num2str(sum(sum(debug.target_covered))/((tsz(1)+2*w)*(tsz(2)+2*w))),'th target patched covered']);
              ...
              for jj = jj_seq
                 if(debug.target_covered(ii,jj)==0)
                        ...
                        while time<=4%10 次·，每次随机选择lenRad个（没use过的SourcePatch+offset小于阈值的，进行替换）
                            ...
                            if debug.source_used(nns(1,idx),nns(2,idx))<K && ofs_rs(idx)<=T
...
%                                     disp([num2str(ii),num2str(jj),' do random selection again']);
    ...
    elseif(sum(sum(debug.target_covered))/((tsz(1)+2*w)*(tsz(2)+2*w))<1)%>0.95 covered
