function [best_app_i, best_app_j] = best_approximate_match_LPE(A_features, ...
  A_pyramid, B_pyramid, B_features, l,height, width)
...
for kB=1:size(idx,2)
   [i, j] = ind2sub([A_h, A_w], idx(kB)); % flip???
   best_app_i=[best_app_i
                        i];%第kB的feature对应 A中的（i,j）中的i
   best_app_j=[best_app_j
                        j];%第kB的feature对应 A中的（i,j）中的j
end
% Free memory
anno = close(anno);
