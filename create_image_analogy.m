...
B_pyramid = {B};
B_prime_pyramid = {zeros(size(B,1),size(B,2),3)};
...
for l=1:L   %Çø¼ä0-255
  A_pyramid_extend{l} = extend_image(A_pyramid{l}, N_BIG/2, [1,4,7,10]);
  B_pyramid_extend{l} = extend_image(B_pyramid{l}, N_BIG/2, [1,4,7,10]);
...
for l = 1:L
  ...
  A_features{l} = zeros(A_height * A_width, NNF*size(A_l,3));
  B_features{l} = zeros(B_height * B_width, NNF*size(B_l,3));
  ...
  for i = 1:A_height
    for j = 1:A_width
      ...
      A_features{l}(sub2ind([A_height A_width], i, j), :) = ...
        [reshape(A_l(i:i+N_BIG-1, j:j+N_BIG-1, 1), 1, NNF)   reshape(A_l(i:i+N_BIG-1, j:j+N_BIG-1, 2), 1, NNF)   reshape(A_l(i:i+N_BIG-1, j:j+N_BIG-1, 3), 1, NNF)   reshape(A_l(i:i+N_BIG-1, j:j+N_BIG-1, 4), 1, NNF) ];
    end
  end
  ...
  [reshape(B_l(i:i+N_BIG-1, j:j+N_BIG-1, 1), 1, NNF)   reshape(B_l(i:i+N_BIG-1, j:j+N_BIG-1, 2), 1, NNF)   reshape(B_l(i:i+N_BIG-1, j:j+N_BIG-1, 3), 1, NNF)   reshape(B_l(i:i+N_BIG-1, j:j+N_BIG-1, 4), 1, NNF) ];
...
%       A_prime_pyramid{l}(best_i, best_j, 1);
      %       if strcmp(type, 'app')
      B_prime_pyramid{l}(i, j, 1:3) = A_prime_pyramid{l}(best_i, best_j, 1:3);
      %       else
